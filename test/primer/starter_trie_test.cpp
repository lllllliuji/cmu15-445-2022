...
TEST(StarterTest, TrieInsertTest) {
  ...
  // Insert different data types
  {
    ...
    success = trie.Insert<int>("aa", 15646);
    EXPECT_EQ(success, false);

    EXPECT_EQ(trie.GetValue<std::string>("aa", &success), "val");
    EXPECT_EQ(success, true);

    EXPECT_EQ(trie.GetValue<int>("aa", &success), 0);
    EXPECT_EQ(success, false);
    
    success = trie.Insert<std::string>("ab", "hello");
    EXPECT_EQ(success, true);

    success = trie.Insert<int>("abcdefghijk", 100);
    EXPECT_EQ(success, true);

    success = trie.Insert<std::string>("abcdef", "hello world");
    EXPECT_EQ(success, true);

    success = trie.Remove("abcdefghijk");
    EXPECT_EQ(success, true);

    EXPECT_EQ(trie.GetValue<std::string>("abcdef", &success), "hello world");
    EXPECT_EQ(success, true);
